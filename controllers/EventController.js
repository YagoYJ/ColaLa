  ...
  async create(req, res) {
    ...
    await joi.validate(req.body, schema, (err, result) => {
      if (err) {
        ...
        if (err.message == 'child "title" fails because ["title" length must be less than or equal to 30 characters long]'){
          req.flash("error_msg", "Título inválido.");
          res.redirect("/new-event");
        } else if (err.message == 'child "description" fails because ["description" is not allowed to be empty]') {
          req.flash("error_msg", "O evento deve possuir uma descrição.");
          res.redirect("/new-event");
        } else if (err.message == 'child "date" fails because ["date" is not allowed to be empty]'){
          req.flash("error_msg", "Data Inválida");
          res.redirect("/new-event");
        } else if (err.message == 'child "hour" fails because ["hour" is not allowed to be empty]'){
          req.flash("error_msg", "Hora Inválida");
          res.redirect("/new-event");
        } else if (err.message == 'child "hour" fails because ["hour" with value "'+req.body.hour+'" fails to match the required pattern: /^(0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/]'){
          req.flash("error_msg", "Hora Inválida");
          res.redirect("/new-event");
        } else if (err.message == 'child "modality" fails because ["date" is not allowed to be empty]') {
          req.flash("error_msg", "Selecione uma modalidade.");
          res.redirect("/new-event");
        } else if (err.message == 'child "private" fails because ["private" is not allowed to be empty]'){
